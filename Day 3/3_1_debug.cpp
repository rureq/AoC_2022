...
void split_str_evenly(const std::string &str, std::string &sub_str_1, std::string &sub_str_2)
{
    if(str.size()%2 == 0){ //check if the lentgh of base string is even
        std::cout<<"size:"<<str.size()<<std::endl; //help for debugging
...
int main()
{
    ...
    std::string test_string = "vJrwpWtwJgWrhcsFMMfFFhFp"; //this and next 4 lines for testing purposes
    split_str_evenly(test_string, sub_str_1, sub_str_2);
    rep = find_repeating_char(sub_str_1,sub_str_2);
    std::cout<<"1: "<<sub_str_1<<std::endl<<"2: "<<sub_str_2<<std::endl;
