...
int main()
{
    ...
    while (!File.eof()) // until the end of file has been reached do:
    {
        ...
        // check four cases of range overlapping - two when only partially and two when one cointans the second one fully
        if ((stoi(input[1]) >= stoi(input[2]) && stoi(input[0]) <= stoi(input[3])) && found != true)
        {
            result++;
            found = true;
        }
        if ((stoi(input[0]) <= stoi(input[3]) && stoi(input[1]) >= stoi(input[2])) && found != true)
        {
            result++;
            found = true;
        }
        if ((stoi(input[0]) >= stoi(input[2]) && stoi(input[0]) <= stoi(input[3])) && found != true)
        {
            result++;
            found = true;
        }
        if ((stoi(input[3]) <= stoi(input[1]) && stoi(input[0]) <= stoi(input[3])) && found != true)
        {
            result++;
            found = true;
        }
