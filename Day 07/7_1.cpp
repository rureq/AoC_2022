...
__int64 subdir_count(std::fstream &file, __int64 &total_size)
{
    ...
    while (getline(file, line))
    {
        ...
        {
            sub_size = subdir_count(file, total_size); // call function
...
int main()
{
    ...
    {
        total_read_size = subdir_count(File, total_size); // run recursive function of counting size of directories, returns directory of root
